...
int main(int argc, char* argv[])
{
    ...
    while (!(last_digit_before == 2 && last_digit_after == 0) && loop)
    {
        ...
        while (pruning)
        {
            ...
            {
                if (pattern_counter[num_genes - 1] == 2 && (best >= relaxation_down[0])) // revised to make operator go from > to >=
                {
                    //cout << "Triggering termination condition at: ";
                    //printCounter(pattern_counter, num_genes); 

                    //incrementCounter(pattern_counter, 3, num_genes, 0); //revised to increment at index 0 instead of num_genes-1
        ...
        //BREAK THE MASTER COMPARISON LOOP IF LANDING ON A ZERO
        if (last_digit_before == 2 && last_digit_after == 0)
        {
            break;
        }
        ...
        //int k = (num_patients - num_cases); k < num_patients; k++
        control_score = 0;
        for (int k = num_cases; k < num_patients; k++) // k corresponds to control cases
        {
            for (int i = num_genes - 1; i >= 0; i--) // i corresponds to rows(genes)
            {
        ...
        //Error-handling for "IsNaN" Values (needed to be altered to compile on Linux)
        if (num_cases == 0)
        {
            case_ratio = 0;
        }
        ...
        {
            printCounter(pattern_counter, num_genes);
            cout << double(case_score) << ", " << double(control_score) << endl;
            cout << case_ratio << ", " << control_ratio << endl;
	...
	//cout << "Base-3 pattern number: " << printCounter(best_pattern, num_genes);
	//outputfile << "Base-3 pattern number: " << printCounter(best_pattern, num_genes);

    cout << "Process took " << (after - before) << " seconds" << endl << endl;
    outputfile << "Process took " << (after - before) << " seconds" << endl << endl;
